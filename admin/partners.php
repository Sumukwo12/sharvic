...
if (isset($_GET['delete'])) {
    ...
    // Get logo filename before deleting record
    $logoQuery = "SELECT logo FROM partners WHERE id = $id";
    $logoResult = $conn->query($logoQuery);
    
    if ($logoResult->num_rows > 0) {
        $logoRow = $logoResult->fetch_assoc();
        $logoPath = "../images/" . $logoRow['logo'];
    ...
    if ($conn->query($deleteQuery) === TRUE) {
        ...
        $deleteError = "Error deleting partner: " . $conn->error;
    }
}
...
<head>
    ...
    <title>Manage Partners - Sharvic East Africa Limited</title>
        ...
        <div class="main-content">
            ...
            <div class="content-box">
                <div class="content-header">
                    <h3>All Partners</h3>
                    <a href="add_partner.php" class="btn"><i class="fas fa-plus"></i> Add New Partner</a>
                </div>
                    ...
                    <tbody>
                        ...
                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                echo "<tr>";
                                echo "<td>" . $row["id"] . "</td>";
                                echo "<td><img src='../images/" . $row["logo"] . "' alt='" . $row["name"] . "' class='logo-thumbnail'></td>";
                                echo "<td>" . $row["name"] . "</td>";
                                echo "<td class='action-buttons'>";
                                echo "<a href='edit_partner.php?id=" . $row["id"] . "' class='btn btn-warning'><i class='fas fa-edit'></i> Edit</a>";
                                echo "<button class='btn btn-danger delete-btn' data-id='" . $row["id"] . "'><i class='fas fa-trash'></i> Delete</button>";
                                echo "</td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='4' style='text-align: center;'>No partners found</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
    ...
    <script>
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                confirmBtn.href = `partners.php?delete=${partnerId}`;
